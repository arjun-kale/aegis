...
export function MissionExportModal({ isOpen, onClose, onReplayPlan }: MissionExportModalProps) {
  ...
  return (
    ...
    >
      <div className="w-[620px] max-w-[calc(100vw-32px)] max-h-[85vh] bg-[#FFFFFF] border border-[#DDE1E6] text-[#1B1F24] font-mono text-xs shadow-2xl flex flex-col">
        {/* Modal Header */}
        <div className="flex items-center justify-between px-4 py-2.5 bg-[#EEF0F3] border-b border-[#DDE1E6]">
        ...
        {/* Tab Navigation */}
        <div className="flex border-b border-[#DDE1E6] bg-[#F1F2F5]">
          <button
            onClick={() => setActiveTab('export')}
            className={`flex-1 py-2 flex items-center justify-center gap-2 font-semibold text-xs border-r border-[#DDE1E6] transition-colors ${
              activeTab === 'export'
                ? 'bg-[#FFFFFF] text-accent-tealText border-b-2 border-b-accent-teal'
                : 'text-[#5B6470] hover:text-[#1B1F24]'
            }`}
          >
        ...
        <div className="flex-1 p-4 overflow-y-auto max-h-[60vh]">
          {activeTab === 'export' ? (
            <div className="space-y-3">
              <div className="flex items-center justify-between text-[11px]">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="flex items-center gap-1.5 px-2 py-1 bg-[#F1F2F5] hover:bg-[#EDEFF2] border border-[#DDE1E6] text-[10px] rounded transition-colors"
                  >
              ...
              <pre className="p-3 bg-[#F1F2F5] border border-[#DDE1E6] rounded text-[10px] text-accent-tealText max-h-80 overflow-y-auto leading-relaxed">
            ...
            <div className="space-y-3">
              {/* File Upload Input */}
              <div className="flex items-center gap-2">
                <label className="flex-1 cursor-pointer flex items-center justify-center gap-2 py-2 px-3 bg-[#F1F2F5] border border-dashed border-[#DDE1E6] hover:border-accent-teal rounded text-[#5B6470] hover:text-[#1B1F24] text-[11px] transition-colors">
              ...
              {/* Raw JSON Paste Area */}
              <div>
                <div className="text-[#5B6470] text-[10px] uppercase mb-1">
                ...
                <textarea
                  ...
                  rows={6}
                  className="w-full p-2.5 bg-[#F1F2F5] border border-[#DDE1E6] rounded text-[10px] text-[#1B1F24] focus:outline-none focus:border-accent-teal font-mono resize-none"
                />
              </div>

              {/* Validation Status Error / Success */}
              {importError && (
                <div className="p-2.5 bg-[#F1F2F5] border border-accent-red text-accent-redText text-[11px] rounded flex items-center gap-2">
              ...
              {parsedPlan && (
                <div className="p-3 bg-accent-green/10 border border-accent-green rounded space-y-2">
                  ...
                  <div className="grid grid-cols-3 gap-2 text-[10px]">
                    <div className="p-1.5 bg-[#F1F2F5] rounded border border-[#DDE1E6]">
                      <div className="text-[#5B6470]">WAYPOINTS:</div>
                      <div className="font-semibold text-[#1B1F24]">
                        {parsedPlan.waypoints.length} pts
                      </div>
                    </div>
                    <div className="p-1.5 bg-[#F1F2F5] rounded border border-[#DDE1E6]">
                      <div className="text-[#5B6470]">GAIT:</div>
                      <div className="font-semibold text-accent-green">
                        {parsedPlan.mission_metadata.gait_profile}
                      </div>
                    </div>
                    <div className="p-1.5 bg-[#F1F2F5] rounded border border-[#DDE1E6]">
                      <div className="text-[#5B6470]">MIN MARGIN:</div>
                      <div className="font-semibold text-accent-green">
